...
$(function () {
  ...
  // manage click event of search on title checkbox
  $(document).on('click', '#searchTitle', function () {
    page = 1;
    if ($('#search_keyword').tagsinput('items').length > 0) {
      update_listing_based_on_filteration();
    }
  });
  ...
  $(document).on('click', '.search_clear', function () {
    ...
    if ($('#autocomplete').tagsinput('items').length > 0) {
      update_flag = true;
    }
  ...
  $(document).on('keyup input paste', '.search_text input', function () {
    page = 1;
  ...
  $(document).on('keydown', '.search_text input', function (e) {
    ...
    if ($(this).parents('.search_text').next('#autocomplete').length <= 0) {
      ...
      } else if (e.keyCode == 50 || e.keyCode == 56) {
        e.preventDefault();
      }
      if (flag) {
        real_search_txt = '';
  ...
  $('.visible_option label input').on('click', function (e) {
    ...
    if ($(this).prop('checked')) {
      ...
      if (val.includes('all')) {
        ...
        if (parents_id == 'checkboxes2' || parents_id == 'checkboxes3') {
  ...
  $(document).on('click', '.clear_all_filter', function (e) {
    ...
    if (!chbx2.includes('all') || !chbx3.includes('all') || !chbx4.includes('all')) {
      flag = true;
    }
  ...
  $(document).on('input propertychange', '#srchkey', function () {
    var key = $(this).val();
    ...
    xhr = $.ajax({
      type: "POST",
      url: SITE_URL + "member/filtermember/" + key,
      data: dataString,
      dataType: 'json',
  ...
  $(document).on('click', '.display_additional', function (e) {
    e.stopPropagation();
    e.preventDefault();
    var id = $(this).attr('data-id');

    if ($('#' + id).is(':visible')) {
      $(this).find('i').removeClass('i-minus-circle-2').addClass('i-plus-circle-2');
      $('#' + id).hide();
      // $('#' + id).collapse('hide');

    } else {
      $(this).find('i').removeClass('i-plus-circle-2').addClass('i-minus-circle-2');
      $('#' + id).show();
      // $('#' + id).collapse('show');
  ...
  $(document).on('click', '.find_professional_dropdown', function () {
    var val = $(this).val();
    ...
    $('.find_professional_dropdown').prop('disabled', true);
    xhr = $.ajax({
      type: "POST",
      url: SITE_URL + "member/ajax_save_users_additional_settings/",
      data: { user_id: val, state: state, type: type },
      dataType: 'json',
      success: function (response) {
        if (response['status'] == 200) {
          $('.find_professional_dropdown').prop('disabled', false);
  ...
  $(document).on('click', '.featured_user_profile', function () {
    var val = $(this).val();
    var type = $(this).attr('data-type');
    var state = '';
    if ($(this).prop('checked')) {
    ...
    $('.featured_user_profile').prop('disabled', true);
    xhr = $.ajax({
      type: "POST",
      url: SITE_URL + "member/ajax_save_featured_profile_on_find_professionals_page/",
      data: { user_id: val, state: state, type: type },
      dataType: 'json',
      success: function (response) {
        if (response['status'] == 200) {
          $('.featured_user_profile').prop('disabled', false);
          $('#msg').html(response['msg']);
          $('#succ').show();
          setTimeout(() => {
            $('#succ').hide();
          }, 3000);
          $('#featured_' + val).html(response['featured_enabled_date']);
        }
      },
      error: function (err) {
        $('.featured_user_profile').prop('disabled', false);
...
function update_listing_based_on_filteration() {
  ...
  var banned_profile = [];
  $.each($('#checkboxes4').children(), function () {
    var value = $(this).find('input').val();
    if (!value.includes('all') && $(this).find('input').prop('checked')) {
      banned_profile.push(value);
    }
  });
  var data = {
    plan: $('#checkboxes2').find('input:checked').val(),
    additional_settings: $('#checkboxes3').find('input:checked').val(),
    banned_profile: banned_profile,
    location: $('#autocomplete').tagsinput('items'),
    searchtxt_arr: $('#search_keyword').tagsinput('items'),
    search_title_flag: $('#searchTitle').prop('checked'),
    real_time_search_txt: real_search_txt,
    search_type: $('.search_type:checked').val()
  };

  xhr = $.ajax({
    url: SITE_URL + 'member/ajax_filter_member/' + page,
    method: 'POST',
    dataType: 'json',
    data: data,
    success: function (response) {
