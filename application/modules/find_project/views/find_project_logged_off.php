							...
							<div>
								...
								<div class="proCat" id="leftSideCatSec">
									...
									<div id="mainCategory" class="chkSameLine">				
										<label class="default_checkbox"><input type="checkbox" id="allCategory" <?php echo $checked; ?>><span class="checkmark"></span><small class="mainCatText <?php echo !empty($checked) ? 'boldCat' : ''; ?>"><?php echo $this->config->item('find_project_project_categories_all_categories_txt'); ?></small></label></div><?php
										}
									?><?php
									...
									?><div class="pmTxt chkSameLine">
										<label class="default_checkbox"><input type="checkbox" id="ckbCheckAlltoggle<?=$key+1?>" value="<?=$parent['id']?>"
												<?php echo (!empty($filter_arr) && is_array($filter_arr['parent_categories']) && in_array($parent['id'], $filter_arr['parent_categories'])) || $flag ? 'checked' : '';?> class="CheckAllClass1 parent"><span class="checkmark"></span><small class="mainCatText <?php echo (!empty($filter_arr) && is_array($filter_arr['parent_categories']) && in_array($parent['id'], $filter_arr['parent_categories'])) || $flag ? 'boldCat' : '';?>"><?php echo $parent['name']; ?></small></label><?php
										...
										?><div class="colTxt1 child" style="display:<?php echo $display_child;?>"><div class="parTab" id="child<?=$val['id']?>">
												<label class="default_checkbox"><input type="checkbox" class="checkBoxClass1" <?php echo (!empty($filter_arr) && is_array($filter_arr['categories']) && in_array($val['id'], $filter_arr['categories'])) ? 'checked' : '';?> data-parent="<?php echo $parent['id']; ?>" value="<?php echo $val['id']; ?>"><span class="checkmark"></span><span class="subCatText <?php echo (!empty($filter_arr) && is_array($filter_arr['categories']) && in_array($val['id'], $filter_arr['categories'])) ? 'boldCat' : '';?>"><?=$val['name']?></span></label></div></div><?php
										...
										?></div><?php
											...
											if(count($categories) >$this->config->item('find_project_maximum_categories_show') ) { echo '<div class="showmore_category"><label class="catAll" onclick="toogleMcat('.$remainingCat.')"><i class="fas fa-angle-down"></i> '.$showmore_category. '</label></div>'; } 
									?></div>
							</div>
						</div>
					</div>
						...
						<div class="row">
							<div class="col-md-8 col-sm-8 col-12 tySrch">
								...
								<div class="receive_notification">
									<!-- <span class="rcv_notfy_btn" data-toggle="collapse" data-target="#rcv_notfy">show more search options <small><i class="fas fa-plus"></i></small></span> -->
				...
				<div class="proDtls">
					...
					?>
						...
						<div class="filter_wrapper">
							<label class="defaultTag"><label class="checkboxes"><?php $upgrades = $this->config->item('find_project_upgrade_list');?><span class="tagFirst"><?php echo $this->config->item('find_project_project_upgrades_dropdown_option_name'); ?></span><small class="tagSecond" style="display:<?php echo (empty($filter_arr) || empty($filter_arr['upgrades']) ? 'block' : 'none !important' ) ?>"><?php echo $this->config->item('find_project_professionals_all_option_txt'); ?><i class="fa fa-times" style="display:none" aria-hidden="true"></i></small><?php
										if(!empty($filter_arr) && !empty($filter_arr['upgrades'])) {
				...
				<!--<div class="pDtls">-->
					<div class="default_project_row <?php echo $featured_class?>">
						...
						echo '<span class="touch_line_break">'.$this->config->item('find_projects_project_payment_method_offline_system').'</span>';
							...
							?><?php
							if($open_bidding_project_value['sealed'] == 'N'){
						...
						<div class="default_project_description desktop-secreen">
							<p><?php 
												   ...
												   // echo limitString($description,$this->config->item('find_project_description_character_limit_desktop'));?></p>
						</div>
						<div class="default_project_description ipad-screen">
							<p><?php 
													echo character_limiter($open_bidding_project_value['project_description'],$this->config->item('find_project_description_character_limit_tablet'));
												   // echo limitString($description,$this->config->item('find_project_description_character_limit_tablet'));?></p>
						</div>
						<div class="default_project_description mobile-screen">
							<p><?php 
													echo character_limiter($open_bidding_project_value['project_description'],$this->config->item('find_project_description_character_limit_mobile'));
													//echo limitString($description,$this->config->item('find_project_description_character_limit_mobile'));?></p>
						</div>
					<!--</div>
					<div class="clearfix"></div>-->
					<?php
					...
					?>
					<div class="row" <?php //if($badgeCount<2 && count($open_bidding_project_value['categories'])>2) { echo 'style="margin-bottom:-5px"';} ?>>
						<div class="col-md-12 col-sm-12 col-12">
							<div class="default_project_category text-center">					
								...
								<button type="button" class="btn badge_sealed"><?php echo $this->config->item('post_project_page_upgrade_type_sealed'); ?></button>
												...
												?> 
													<button type="button" data-page-type-attr = "listing" data-attr="<?php echo $open_bidding_project_value['project_id']; ?>" class="btn default_btn blue_btn login apply_now_button"><?php echo $apply_now_button_text; ?></button>
												</div>
											</div>
										<?php } ?>
										</div>-->
					...
					<div class="pDBttm" <?php //if($open_bidding_project_value['featured'] != 'Y' && $open_bidding_project_value['urgent'] != 'Y' && $open_bidding_project_value['sealed'] != 'Y' && count($open_bidding_project_value['categories'])<3) { echo 'style="margin-bottom:2px"';} ?>>
...
<script src="<?php echo JS; ?>modules/find_project_logged_off.js"></script>
...
<script>
	...
	$(function() {
	<?php 
			if($this->session->userdata('login_popup_op')) {
	?>
