							...
							<div>
								...
								<div class="proCat" id="leftSideCatSec">
									...
									<div id="mainCategory" class="chkSameLine">
										<label class="default_checkbox"><input type="checkbox" id="allCategory"
												<?php echo $checked; ?>><span class="checkmark"></span><small class="mainCatText <?php echo !empty($checked) ? 'boldCat' : '' ?>"><?php echo $this->config->item('find_professionals_professionals_categories_all_categories_txt'); ?></small></label></div><?php
										}
									?><?php
									...
									foreach ($categories as $key => $parent) {
										$m++;
										if ($m == ($this->config->item('find_professionals_maximum_categories_show') + 1)) {echo '<input type="hidden" class="moreCat" value="1"><input type="hidden" class="rCat" value="' . $remainingCat . '"><section id="catDiv"  class="chkSameLine" style="display:none">';}
									...
									?><div class="pmTxt chkSameLine">
										<label class="default_checkbox"><input type="checkbox" id="ckbCheckAlltoggle<?php echo $key + 1; ?>" value="<?php echo $parent['id']; ?>" <?php echo (!empty($filter_arr) && is_array($filter_arr['parent_categories']) && in_array($parent['id'], $filter_arr['parent_categories'])) || $flag ? 'checked' : ''; ?>
											class="CheckAllClass1 parent"><span class="checkmark"></span><small class="mainCatText <?php echo (!empty($filter_arr) && is_array($filter_arr['parent_categories']) && in_array($parent['id'], $filter_arr['parent_categories'])) || $flag ? 'boldCat' : ''; ?>"><?php echo $parent['name']; ?></small></label><?php
					...
					<div class="row">
						<div class="col-md-6 col-sm-6 col-12 searchSection">
							<div class="form-group">
								...
								?>
								<input type="text" class="form-control default_input_field hideField" id="search_keyword" value="<?php echo $search_txt; ?>" data-role="tagsinput" placeholder="<?php echo empty($search_txt) ?  $this->config->item('find_professionals_search_keyword_placeholder') : ''; ?>">
							</div>
						</div>
						<div class="col-md-6 col-sm-6 col-12 pLt0 searchSection">
							<div class="input-group">
								<input type="text" class="form-control" id="autocomplete" placeholder="<?php echo $this->config->item('find_professionals_search_locality_placeholder'); ?>">
						...
						<div class="row">
							<div class="col-md-8 col-sm-8 col-12 tySrch">
								...
								<div class="receive_notification">
									<!--<span class="rcv_notfy_btn" data-toggle="collapse" data-target="#rcv_notfy">show more search options <small>( + )</small></span>-->
									<!-- <a class="rcv_notfy_btn" onclick="showMoreSearch()">show more search options <small>( + )</small></a> -->
						...
						<div class="filter_wrapper">
							<label class="defaultTag"><label class="checkboxes2"><?php 
								...
								<label class="checkboxes3"><span class="tagFirst"><?php echo $this->config->item('find_professionals_hourly_rate_dropdown_option_name'); ?></span><small class="tagSecond" style="display:<?php echo (empty($filter_arr) || empty($filter_arr['hourly_rate_range']) ? 'block' : 'none !important') ?>"><?php echo $this->config->item('find_project_professionals_all_option_txt'); ?><i class="fa fa-times" aria-hidden="true" style="display:none"></i></small><?php 
									...
									?></label></label><label class="defaultTag"><button class="btn default_btn blue_btn btnBold clear_all_filters clear_all_filter"><?php echo $this->config->item('find_professionals_clear_filter_btn_text'); ?></button></label>
						</div>
					</div>
				</div>
				</div>
...
var find_professionals_show_less_subcategories_text = '<?php echo $this->config->item("show_less_txt"); ?>';
var find_professionals_show_more_categories_text = '<?php echo $this->config->item("find_professionals_show_more_categories_text"); ?>';
var find_professionals_show_less_categories_text = '<?php echo $this->config->item("show_less_txt"); ?>';
...
<script src="<?php echo JS; ?>modules/find_professionals_logged_off.js"></script>
...
?>
$(function() {
	// $('input[data-role=tagsinput]').tagsinput('add', '<?php echo $search_txt; ?>');
	<?php
if (!empty($hire_me_user_id)) {
    ?>
	hire_me_user_id = '<?php echo $hire_me_user_id; ?>';
	if($(".default_applyNow_btn .login_popup[data-id="+hire_me_user_id+"]").length == 1 && $(".default_applyNow_btn .login_popup[data-id="+hire_me_user_id+"]").css("display") == 'block') {
		$(".default_applyNow_btn .login_popup[data-id="+hire_me_user_id+"]").trigger('click');
	}
	if($(".contactTab .login_popup[data-id="+hire_me_user_id+"]").length == 1 && $(".contactTab .login_popup[data-id="+hire_me_user_id+"]").css("display") == 'block') {
		$(".contactTab .login_popup[data-id="+hire_me_user_id+"]").trigger('click');
	}
	if($(".contactMobile .login_popup[data-id="+hire_me_user_id+"]").length == 1 && $(".contactMobile .login_popup[data-id="+hire_me_user_id+"]").css("display") == 'block') {
		$(".contactMobile .login_popup[data-id="+hire_me_user_id+"]").trigger('click');
	}
	<?php
}
?>
