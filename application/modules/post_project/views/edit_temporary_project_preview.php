...
if(!empty($temp_category_data)){
	foreach($temp_category_data as $temp_key){
		$selected_category_array[] = $temp_key['temp_project_category_id'];
	}
}
...
if($this->session->userdata('user')){
	...
	if($temp_project_data['featured'] == 'Y'){
	...
	if($temp_project_data['hidden'] == 'Y'){
		if($user_membership_plan_details->included_number_hidden_upgrades != '-1' && $count_user_hidden_membership_included_upgrades_monthly >= $user_membership_plan_details->included_number_hidden_upgrades){
	...
	if(!empty($temp_project_data['featured']) &&  $temp_project_data['featured'] == 'Y') {
		array_push($upgrades, 'featured');
	}
	if(!empty($temp_project_data['urgent']) &&  $temp_project_data['urgent'] == 'Y' ) {
		array_push($upgrades, 'urgent');
	}
	if(!empty($temp_project_data['sealed']) &&  $temp_project_data['sealed'] == 'Y') {
		array_push($upgrades, 'sealed');
	}
	if(!empty($temp_project_data['hidden']) && $temp_project_data['hidden'] == 'Y' ) {
			...
			<div class="row">
				<div class="pProjectLeft">
					...
					<div class="default_block_header_transparent nBorder post_project_header">
						<div class="transparent transparent_heading" id="post_project_page_heading"><?php 
							if(! $this->session->userdata('repost')){
								echo $temp_project_data['project_type']== 'fulltime' ? $this->config->item('post_project_page_heading_edit_fulltime_project_preview') : $this->config->item('post_project_page_heading_edit_project_preview');
							}
							else{
								echo $temp_project_data['project_type']== 'fulltime' ? $this->config->item('post_project_page_heading_post_fulltime') : $this->config->item('post_project_page_heading_post_project');
							}?></div>
						

						<!-- check box button -->
						<div class="checkbox-btn-sectn block-sectn postProject_checkboxBtn create_project_job" data-container="body" data-toggle="popover" data-placement="right" data-content="<?php echo $this->config->item('post_project_page_hover_tooltip_message_post_project_type_option_description_section'); ?>">
							<div class="row">
								<div class="col-md-12 col-sm-12 col-12">
								  ...
								  <!-- <div class="form-group" style="margin:0;">
									<div class="checkbox-btn-inner">
									  <input id="post_project" name="project_type_main" class="post_project_input" style="position: absolute;top: 0;width: 100%;" type="radio" value="post_project">
								...
								<!-- <div class="col-sm-6">
								  <div class="form-group" style="margin:0;">
									<div class="checkbox-btn-inner">
									  <input id="post_fulltime_position" class="post_project_input" name="project_type_main" style="position: absolute;top: 0;width: 100%;" type="radio" value="post_fulltime_position">
									  <div class="checkbox-inner-div">
								...
								<!-- <div class="hover-tooltip-sectn bottom-0 top-auto" style="display:none;">
								  <?php //echo $this->config->item('post_project_page_hover_tooltip_message_post_project_type_option_description_section'); ?>
								</div> -->
							</div>
						</div>
						...
						<!-- check box button -->
						<div class="checkbox-btn-sectn block-sectn postProject_checkboxBtn paymnt_perhour" id="project_type_block" data-container="body" data-toggle="popover" data-placement="right" data-content="<?php echo $this->config->item('post_project_page_hover_tooltip_message_post_project_fixed_hourly_type_option_description_section'); ?>">
							<div class="row">
								<div class="col-md-12 col-sm-12 col-12">
									<div class="default_radio_button">
										<section>
						...
						<div class="block-sectn categories-select-sectn pCategory_first" data-container="body" data-toggle="popover" data-placement="right" data-content="<?php 
							...
							<div class="row">
								...
								{
									$category_counter = 0;
									foreach($temp_category_data as $temp_category)
									{
								?>
										<div class="row category_row" id="<?php echo "project_category_row_".$temp_category['id']."_".$temp_category['temp_project_id']; ?>">	
										<?php
											
											if(empty($temp_category['temp_project_parent_category_id'])){
											
												$get_project_child_categories = $CI->Post_project_model->get_project_child_categories($temp_category['temp_project_category_id']);
											
											}else{
											
												$get_project_child_categories = $CI->Post_project_model->get_project_child_categories($temp_category['temp_project_parent_category_id']);
											}
												  ...
												  <select name="project_category[<?php echo $category_counter ?>][project_parent_category]" id="<?php echo "project_parent_category_".$category_counter ?>" class="project_parent_category">
													...
													if(!empty($project_parent_categories)){
														foreach ($project_parent_categories as $project_parent_category_row){ 
															...
															if($temp_category['temp_project_parent_category_id'] == 0){
																$selected_parent_category_id = $temp_category['temp_project_category_id'];
															}else{
																$selected_parent_category_id = $temp_category['temp_project_parent_category_id'];
																}
															...
															if(!empty($selected_category_array)){
																$array_diff = array();
																$array_diff = array_diff($selected_category_array,array($temp_category['temp_project_category_id']));
												  ...
												  <select name="project_category[<?php echo $category_counter ?>][project_child_category]"  id="<?php echo "project_child_category_".$category_counter ?>"  <?php echo $project_child_category_disabled; ?> class="project_child_category">
													...
													if(!empty($get_project_child_categories )){
														foreach ($get_project_child_categories as $project_child_category_row) {
															...
															if($temp_category['temp_project_parent_category_id'] != 0){
																$selected_child_category_id = $temp_category['temp_project_category_id'];
															}
										...
										<?php if($category_counter != 0){ ?>
										 <a style="cursor:pointer" data-id = "<?php echo $temp_category['temp_project_category_id'] ?>"  id="<?php echo $temp_category['id']."_".$temp_category['temp_project_id'] ?>" class="default_icon_red_btn delete-category-row delete_project_category_row_data"><i class="fas fa-trash-alt"></i></a>
								...
								if($this->config->item('number_project_category_post_project') > count($temp_category_data)  &&  $count_available_project_parent_category_count > count($temp_category_data)   ){
								...
								<div class="col-sm-6 addCategory_btn add_more_project_category_section" style="<?php echo $show_category_button_status; ?>">
									<button type="button" class="btn btn-block blue_btn default_btn addAnCat" id="add_more_project_category" <?php echo $catgory_button_status; ?>>
									<?php
									if(empty($temp_category_data)){
							...
							<div class="row">
								...
								<div class="col-sm-6 project_select_budget">
									<div class="form-group default_dropdown_select margin_bottom0 error_div_height_auto">
										...
										if($temp_project_data['confidential_dropdown_option_selected'] == 'Y'){
										
											$budget_selected_value = key($this->config->item('fixed_budget_projects_confidential_dropdown_option'));
										
										}else if($temp_project_data['not_sure_dropdown_option_selected'] == 'Y'){
											$budget_selected_value = key($this->config->item('fixed_budget_projects_not_sure_dropdown_option'));
										}
										else if(!empty($temp_project_data['min_budget'])){
											$budget_selected_value = $temp_project_data['min_budget']."_".$temp_project_data['max_budget'];
										}
										?> 
								...
								<div class="col-sm-6 project_select_continue">
									<!--<button type="button" class="btn default_btn btn-block blue_btn addAnCat" id="project_next_button" disabled style="display:none;"><?php //echo $this->config->item('post_project_page_next_button_txt'); ?></button>-->
								</div>
						...
						<!-- Project name -->
						<div class="block-sectn file-project-name project_show_block projectName_section" id="project_title_section" data-placement="right" data-content="<?php
							if($temp_project_data['project_type'] == 'fulltime'){ 
							...
							<div class="row">
								...
								<div class="col-md-12">
									<div class="form-group projectName"> 
										<?php
										$project_title = $temp_project_data['project_title'];
										...
										?>
										<input type="text" name="project_title" id="project_title" class="avoid_space project_title default_input_field" maxlength="<?php echo $this->config->item('project_title_maximum_length_characters_limit_post_project'); ?>" value="<?php echo $project_title; ?>"> 
						...
						<div id="project_description_section" class="block-sectn project-describe project_show_block noBorder_block_sectn projectDescription_section" data-container="body" data-toggle="popover" data-placement="right" data-content="<?php
								if($temp_project_data['project_type'] == 'fulltime'){ 
							...
							<div class="row">
								<div class="col-md-12">
									<h4 class="inner-block-heading default_black_bold_large">
										<span id="project_description_section_heading">
										<?php
										if($temp_project_data['project_type'] == 'fulltime'){ 
								...
								<div class="col-md-12">
									<div class="form-group margin0"> 
										<?php
											$project_description = trim($temp_project_data['project_description']);
											...
											// }
											//pre($project_description_remaining_characters);
											if($this->config->item('project_description_maximum_length_characters_limit_post_project') - mb_strlen(trim($project_description)) >= 0){
												$project_description_remaining_characters = ($this->config->item('project_description_maximum_length_characters_limit_post_project') - mb_strlen(trim($project_description)));
											} else {
												$project_description_remaining_characters = 0;
							...
							<div class="row">
								<div class="col-md-12">
									<div class="upload-btn-wrapper pPfileInput_btn">
										...
										</span><div class="upload_attachment_error errAttachment"></div><!-- <div id="project_attachment_container" class="attachment_inline" style="display:none;"></div> --><?php
										...
										if(!empty($project_attachment_array)){
											foreach($project_attachment_array as $temp_project_attachment){
												$attachment_id = Cryptor::doEncrypt($temp_project_attachment['id']);
											?><div class="default_download_attachment project_attachment_row" id="<?php echo "temp_project_attachment_row".$temp_project_attachment['id']; ?>"><a class="download_attachment download_text" data-attr="<?php echo $attachment_id; ?>"><label><?php echo $temp_project_attachment['temp_project_attachment_name']; ?></label></a><label class="delete_icon 999"><a href="javascript:;" class="temp_project_attachment_row" data-file-attr = "<?php echo $temp_project_attachment['temp_project_attachment_name']; ?>" id="<?php echo $temp_project_attachment['id'];?>"><i class="fa fa-trash-o delete-btn" aria-hidden="true" ></i></a></label><div class="clearfix"></div></div><?php
									...
									<table class="table" id="project_attachment_container" style="<?php //echo $show_attachment_table; ?>">
										...
										/*if(!empty($project_attachment_array)){
											foreach($project_attachment_array as $temp_project_attachment){
												$attachment_id = Cryptor::doEncrypt($temp_project_attachment['id']);
										?>
											<tr class="project_attachment_row" id="<?php echo "temp_project_attachment_row".$temp_project_attachment['id']; ?>">
												<td ><a class="download_attachment" style="cursor:pointer" data-attr="<?php echo $attachment_id; ?>" ><?php echo $temp_project_attachment['temp_project_attachment_name']; ?></a></td>
												<td align="center"><?php echo $temp_project_attachment['size']; ?></td>
												<td  align="center">
													<a style="cursor:pointer;" class="btn default_icon_red_btn temp_project_attachment_row" id="<?php echo $temp_project_attachment['id'];?>"> <i class="fas fa-trash-alt"></i> </a>	  
												</td>
											</tr>
										<?php
											}
						...
						<!-- tags -->
						<div id="project_tag_section"  class="block-sectn tags-sectn project_show_block" data-container="body" data-toggle="popover" data-placement="right" data-content="<?php
								if($temp_project_data['project_type'] == 'fulltime'){ 
							...
							<div class="row">
								<div class="col-md-12">
									<div class="receive_notification" id="tagSL">
										...
										<?php if(!empty($temp_project_tag_data)){
											echo "( - )"; } else {echo "( + )";}
										?>
										</small>
										</a>
										<input type="hidden" id="moreTags" value="<?php if(!empty($temp_project_tag_data)){
											echo "0"; } else {echo "1";}
										?>">
									</div>
									<div id="more_tags" class="<?php if(empty($temp_project_tag_data)){
											echo "collapse"; }
										?> row">
										<!-- <h4 class="inner-block-heading default_black_bold_large">
											<span id="project_tag_heading_section">
											<?php
											/*if($temp_project_data['project_type'] == 'fulltime'){ 
												echo $this->config->item('post_fulltime_position_tags_section_heading');
											}else{
										...
										<!-- </div> -->
										<div class="tagBottom">
											<input type="text" id="input_tags" name=""  class="avoid_space avoid_space default_input_field" maxlength="<?php echo $this->config->item('project_tag_maximum_length_characters_limit_post_project'); ?>" >
											...
											<ul id="tags-list" class="default_cross_tag"> 
													...
													if(!empty($temp_project_tag_data)){
															$tag_counter = 0;
															foreach($temp_project_tag_data as $temp_project_tag){

													?>

															<li class="tag_name" id="<?php echo 'project_tag_'.$temp_project_tag['id']."_".$temp_project_tag['temp_project_id'] ?>"> <span><small><?php echo $temp_project_tag['temp_project_tag_name']; ?></small><input type="hidden" name="project_tag[<?php echo $tag_counter; ?>][tag_name]" value="<?php echo $temp_project_tag['temp_project_tag_name']; ?>" /><i class="fa fa-times delete_project_tag_row_data" data-attr="<?php echo 'project_tag_'.$temp_project_tag['id']."_".$temp_project_tag['temp_project_id'] ?>"></i> </span></li>
							...
							<div class="row">
								<div class="col-md-12">
									<div class="receive_notification" id="locationSL">					
										<a class="rcv_notfy_btn location_option chk-btn" onclick="showMorePow()" id="location_heading">
										<?php
										if($temp_project_data['project_type'] == 'fulltime'){ 
										...
										if(!empty($temp_project_data['county_id'])){
											echo " ( - )";
										}else{
											echo " ( + )";
										}
										...
										<input type="hidden" id="morePow" name="location_option" value="<?php
										if(!empty($temp_project_data['county_id'])){
											echo "0";
										}else{
											echo "1";
									...
									<div id="more_pow" class="collapse location_section">
										<div class="row">
											<div class="col-md-5 col-sm-5 col-12 placeWork">
												<div class="form-group default_dropdown_select"> 
													<select id="project_county_id" name="project_county_id">
														...
														<option <?php echo isset ($temp_project_data['county_id']) && $temp_project_data['county_id'] == $county['id'] ? 'selected' : ''; ?> value="<?php echo $county['id']; ?>"><?php echo $county['name'] ?></option>
														<?php endforeach; ?>
													</select> 
													<div class="error_div_sectn clearfix">
											...
											<div class="col-md-5 col-sm-5 col-12 selectMunicipility" id="selectMunicipility" style="<?php if($temp_project_data['county_id'] != 0 && !empty($localities)){ echo "display:block;";} else { echo "display:none;";} ?>">
												<div class="form-group default_dropdown_select"> 
													<?php
													...
													<select  name="project_locality_id" id="project_locality_id" <?php echo $lacality_disabled;?> >
														...
														<option <?php echo isset ($temp_project_data['locality_id']) && $temp_project_data['locality_id'] == $locality['id'] ? 'selected' : ''; ?>
											...
											<div class="col-md-2 col-sm-2 col-12 selectPostcode" id="selectPostcode" style="<?php if($temp_project_data['locality_id'] != 0 && !empty($postal_codes)){ echo "display:block;";} else { echo "display:none;";} ?>">
												<div class="form-group default_dropdown_select"> 
													<?php
													$postal_code_disabled = "disabled";
													if(!empty($temp_project_data['locality_id'])){
													...
													<select name="project_postal_code_id" id="project_postal_code_id" <?php echo $postal_code_disabled;?>>
														...
														<option <?php echo isset ($temp_project_data['postal_code_id']) && $temp_project_data['postal_code_id'] == $postal_code['id'] ? 'selected' : ''; ?>
						...
						<!-- Where do you want this done? -->
						<!-- Payment Methods -->
						<div class="payment-methods block-sectn project_show_block" id="payment_method_section" data-container="body" data-toggle="popover" data-placement="right" data-content="<?php echo $this->config->item('post_project_page_hover_tooltip_message_payment_method_section'); ?>">
							<div class="row">
								<div class="col-md-12">
									<div class="receive_notification" id="pmethodSL">					
										...
										<?php if($temp_project_data['escrow_payment_method'] == 'Y' || $temp_project_data['offline_payment_method'] == 'Y'){
											echo "( - )";
											}else{
											echo "( + )";
										}?></small></a>
										<input type="hidden" id="morePmethod" value="<?php if($temp_project_data['escrow_payment_method'] == 'Y' || $temp_project_data['offline_payment_method'] == 'Y'){
											echo "0";
										}else{
											echo "1";
										}
									...
									<!-- <div class="col-md-12">
										<h4 class="inner-block-heading default_black_bold_large">
										<span id="project_payment_method_section_heading">
										<?php
										/*if($temp_project_data['project_type'] == 'fulltime'){ 
									...
									<div id="more_pmethod" class="<?php if($temp_project_data['escrow_payment_method'] == 'N' && $temp_project_data['offline_payment_method'] == 'N'){
											echo "collapse";
										}
										?> row paymentMethod">
										<div class="col-md-12 col-sm-12 col-12 fontSize0">
						...
						<div class="most-project block-sectn-without-blue-tooltip project_show_block">
						  <div class="row">
							  <div class="col-md-12">
								<h4 class="inner-block-heading default_black_bold_large improveAD"><span id="project_optional_upgrades_section_heading"><?php
									if($temp_project_data['project_type'] == 'fulltime'){ 
							 ...
							 <div class="form-group">
								  ...
								  <input id="most-project2"  class="upgrade_type_urgent upgrade_type" style="position: absolute;top: 0;width: 100%;" type="checkbox" name="upgrade_type_urgent"  value="Y">
									<div class="checkbox-inner-div">
										<label for="most-project2">
											...
											<div class="checkbox-content">
											  <p id="urgent_description" class="upgrade_badge_description badge_bigger_view"><?php echo $temp_project_data['project_type']== 'fulltime' ? str_replace(array('{project_urgent_upgrade_availability}'),array($urgent_availabity_days),$this->config->item('post_fulltime_project_page_project_upgrade_description_urgent')) : str_replace(array('{project_urgent_upgrade_availability}'),array($urgent_availabity_days),$this->config->item('post_project_page_project_upgrade_description_urgent'));?></p>
                                               <p id="urgent_description" class="upgrade_badge_description badge_smaller_view"><?php echo $temp_project_data['project_type']== 'fulltime' ? str_replace(array('{project_urgent_upgrade_availability}'),array($urgent_availabity_days),$this->config->item('post_fulltime_project_page_project_upgrade_description_urgent_small_resolution_view')) : str_replace(array('{project_urgent_upgrade_availability}'),array($urgent_availabity_days),$this->config->item('post_project_page_project_upgrade_description_urgent_small_resolution_view'));?></p>
											</div>
										</label>
									</div>
								</div>
								  ...
								  <input id="most-project3"  class="upgrade_type_sealed upgrade_type" style="position: absolute;top: 0;width: 100%;" type="checkbox" name="upgrade_type_sealed" value="Y" >
									<div class="checkbox-inner-div">
										<label for="most-project3">
											...
											<div class="checkbox-content">
											  <p id="sealed_description" class="upgrade_badge_description badge_bigger_view"><?php echo $temp_project_data['project_type']== 'fulltime' ? $this->config->item('post_fulltime_project_page_project_upgrade_description_sealed') : $this->config->item('post_project_page_project_upgrade_description_sealed');?></p>
                                               <p id="sealed_description" class="upgrade_badge_description badge_smaller_view"><?php echo $temp_project_data['project_type']== 'fulltime' ? $this->config->item('post_fulltime_project_page_project_upgrade_description_sealed_small_resolution_view') : $this->config->item('post_project_page_project_upgrade_description_sealed_small_resolution_view');?></p>
											</div>
										</label>
									</div>
								</div>
								  ...
								  <input id="most-project4"  class="upgrade_type_hidden upgrade_type" style="position: absolute;top: 0;width: 100%;" type="checkbox" name="upgrade_type_hidden" value="Y">
									<div class="checkbox-inner-div">
										<label for="most-project4">
											...
											<div class="checkbox-content">
											  <p id="hidden_description" class="upgrade_badge_description badge_bigger_view"><?php echo $temp_project_data['project_type']== 'fulltime' ? $this->config->item('post_fulltime_project_page_project_upgrade_description_hidden') : $this->config->item('post_project_page_project_upgrade_description_hidden');?></p>
                                               <p id="hidden_description" class="upgrade_badge_description badge_smaller_view"><?php echo $temp_project_data['project_type']== 'fulltime' ? $this->config->item('post_fulltime_project_page_project_upgrade_description_hidden_small_resolution_view') : $this->config->item('post_project_page_project_upgrade_description_hidden_small_resolution_view');?></p>
											</div>
										</label>
									</div>
								</div>
							  ...
							  <div class="total-price" style="display:none;border-top:none;">
								...
								// for featured
								if($temp_project_data['featured'] == 'Y' && ($user_membership_plan_details->included_number_featured_upgrades != '-1' && $count_user_featured_membership_included_upgrades_monthly < $user_membership_plan_details->included_number_featured_upgrades )){
								...
								if($temp_project_data['featured'] == 'Y' && $user_membership_plan_details->included_number_featured_upgrades == '-1'){
									if($temp_project_data['project_type']== 'fulltime'){
								...
								// for urgent
								if($temp_project_data['urgent'] == 'Y' && ($user_membership_plan_details->included_number_urgent_upgrades != '-1' && $count_user_urgent_membership_included_upgrades_monthly < $user_membership_plan_details->included_number_urgent_upgrades)){
								...
								if($temp_project_data['urgent'] == 'Y' && $user_membership_plan_details->included_number_urgent_upgrades == '-1'){
									if($temp_project_data['project_type']== 'fulltime'){
								...
								// for sealed
								if($temp_project_data['sealed'] == 'Y' && ($user_membership_plan_details->included_number_sealed_upgrades != '-1' && $count_user_sealed_membership_included_upgrades_monthly < $user_membership_plan_details->included_number_sealed_upgrades)){
								...
								if($temp_project_data['sealed'] == 'Y' && $user_membership_plan_details->included_number_sealed_upgrades == '-1'){
									if($temp_project_data['project_type']== 'fulltime'){
								...
								// for hidden
								if($temp_project_data['hidden'] == 'Y' && ($user_membership_plan_details->included_number_hidden_upgrades != '-1' && $count_user_hidden_membership_included_upgrades_monthly < $user_membership_plan_details->included_number_hidden_upgrades)){
									...
									if($remaining_hidden_upgrades > 0){
										if(($remaining_hidden_upgrades-1) > 1){
											...
											if($temp_project_data['project_type']== 'fulltime'){
												$hidden_upgrades_included_membership_available_disclaimer = $this->config->item('post_fulltime_project_disclaimer_last_free_sealed_upgrade_included_membership_available');
												}else{
												$hidden_upgrades_included_membership_available_disclaimer = $this->config->item('post_project_disclaimer_last_free_sealed_upgrade_included_membership_available');
											}
										}
								...
								if($temp_project_data['hidden'] == 'Y' && $user_membership_plan_details->included_number_hidden_upgrades == '-1'){
									if($temp_project_data['project_type']== 'fulltime'){
						...
						<div class="project-relative-btn block-sectn-without-blue-tooltip project_show_block <?php
echo $temp_project_data['project_type']== 'fulltime' ? 'tempFulltimeBtn' : ''; ?>">
						  <div class="row">
							<div class="col-md-12 text-center pPBtn_gap">
								<button type="button" class="btn default_btn red_btn" id="post_project_cancel"><?php echo $temp_project_data['project_type']== 'fulltime' ? $this->config->item('post_fulltime_project_page_cancel_project_button_txt') : $this->config->item('post_project_page_cancel_project_button_txt');?></button>
								<button type="button" class="btn default_btn blue_btn" id="post_project_preview"><?php echo $temp_project_data['project_type'] == 'fulltime' ? $this->config->item('post_fulltime_project_page_preview_project_button_txt') :$this->config->item('post_project_page_preview_project_button_txt');?></button>
															 
								<button type="button" class="btn default_btn blue_btn" id="post_project_draft"><?php echo $temp_project_data['project_type'] == 'fulltime' ? $this->config->item('post_fulltime_project_page_save_project_as_draft_button_txt') : $this->config->item('post_project_page_save_project_as_draft_button_txt');?></button>
								<button type="button" class="btn green_btn default_btn" id="post_project_publish"><?php echo $temp_project_data['project_type'] == 'fulltime' ? $this->config->item('post_fulltime_project_page_publish_project_button_txt') : $this->config->item('post_project_page_publish_project_button_txt');?></button>
							</div>
						  </div>
						</div>
						<!-- project relative buttons -->
				...
				<!--<div class="pProjectRight hidden-mobile" style="opacity: inherit;z-index: -1;">
...
if($this->session->userdata('user')){
	...
	?>
	<?php
	if((($fulltime_draft_cnt < $po_max_draft_fulltime_projects_number && $po_max_draft_fulltime_projects_number != 0) || ($fulltime_open_bidding_cnt < $po_max_open_fulltime_projects_number && (!empty($standard_valid_time_arr) || $po_max_open_projects_number != '0')))){
	?>
...
//////////////////////////////config for edit temporary project preview////////////////////////
var post_project_page_heading_edit_project = "<?php echo $this->config->item('post_project_page_heading_edit_project_preview'); ?>";
var post_project_page_heading_edit_fulltime_project = "<?php echo $this->config->item('post_project_page_heading_edit_fulltime_project_preview'); ?>";
...
?>	
	...
	$('#fulltime_salary_range option[value=""]').css('display','none');
...
<?php
		if($temp_project_data['project_type'] == 'fixed'){
?>		
		...
		$("#hourly_rate_based_budget").attr('disabled','disabled');			
<?php
		}else if($temp_project_data['project_type'] == 'hourly'){
?>
		$('#project_type_hourly').next('label').css({ 'pointer-events': 'none' });	
		$('#hourly_rate_based_budget option[value=""]').css('display','none');
...
?>
	...
	if($temp_project_data['escrow_payment_method'] == 'Y'){
	?>
		$(".escrow_payment_method").prop("checked", true);	
	<?php
	}if($temp_project_data['offline_payment_method'] == 'Y'){
	?>
	...
	if(!empty($temp_project_data['postal_code_id'])){
	?>
		//$('#project_postal_code_id option[value=""]').css('display','none');
	<?php
	}
	if(!empty($temp_project_data['min_budget'])){
	?>
		$('#fixed_budget option[value=""]').css('display','none');
	<?php
	}
	...
	if($temp_project_data['county_id'] == 0){
	?>
	$('#project_county_id option[value=""]').css('display','none');
	<?php	
	}
	if($temp_project_data['locality_id'] == 0){
	?>
	$('#project_locality_id option[value=""]').css('display','none');
	<?php
	}
	if($temp_project_data['project_postal_code_id'] == 0){
	?>
	$('#project_postal_code_id option[value=""]').css('display','none');
	<?php
	}
...
<script src="<?php echo JS; ?>modules/edit_temporary_project_preview.js"></script>
