<?php
	...
	if(!empty($project_notifications)) {
		foreach($project_notifications as $open_bidding_project_key => $open_bidding_project_value){
...
<div class="default_project_row <?php echo $featured_class?>">
		...
		<label class="default_short_details_field default_continue_text">
			<small>
				<?php 
					if($open_bidding_project_value['account_type'] == USER_PERSONAL_ACCOUNT_TYPE || ($open_bidding_project_value['account_type'] == USER_COMPANY_ACCOUNT_TYPE && $open_bidding_project_value['is_authorized_physical_person'] == 'Y')) {
						if($open_bidding_project_value['gender'] == 'M') {
							echo '<i class="fas fa-male"></i>'.$open_bidding_project_value['first_name'].' '.$open_bidding_project_value['last_name'];
						} else {
							echo '<i class="fas fa-female"></i>'.$open_bidding_project_value['first_name'].' '.$open_bidding_project_value['last_name'];
						}
					} else {
						echo '<i class="fas fa-building"></i>'.$open_bidding_project_value['company_name'];
					}
		...
		<div class="default_project_description desktop-secreen">
			<p><?php 
			echo character_limiter($open_bidding_project_value['project_description'],$this->config->item('newly_posted_projects_description_character_limit_desktop'));?></p>
		</div>
		<div class="default_project_description ipad-screen">
			<p><?php 
			echo character_limiter($open_bidding_project_value['project_description'],$this->config->item('newly_posted_projects_description_character_limit_tablet'));?></p>
		</div>
		<div class="default_project_description mobile-screen">
			<p><?php 
			echo character_limiter($open_bidding_project_value['project_description'],$this->config->item('newly_posted_projects_description_character_limit_mobile'));?></p>
		</div>
						<!-- </div>
...
<div class="row">
	<div class="col-md-12 col-sm-12 col-12">
				...
				foreach($open_bidding_project_value['categories'] as $category_key=>$category_value){
					if($category_key < $this->config->item('find_projects_categories_show_more_less')){
						...
						<div class="clearfix catSub12">
							...
							<a class="cat" data-id="<?php echo $category_value['c_id']; ?>">
						...
						<?php
							...
							echo '<small class="cat" data-id="'.$category_value['p_id'].'">'.$category_value['parent_category_name'].'</small>'; 
						}
				} else {
					//if($category_key == 2){
					?>
					...
					<?php } else if(!empty($category_value['category_name'])) { ?>
						<small class="read-more-target cat" data-id="<?php echo $category_value['c_id']; ?>"><?php echo $category_value['category_name']; ?></small>
					<?php } else if(!empty($category_value['parent_category_name'])) { ?>
						<small class="read-more-target cat" data-id="<?php echo $category_value['p_id']; ?>"><?php echo $category_value['parent_category_name']; ?></small>
					<?php } ?> 
		...
		<?php //if($badgeCount<2 && count($open_bidding_project_value['categories'])>$this->config->item('find_projects_categories_show_more_less')) {  ?>
			<!-- <div class="default_project_socialicon_adjust fontSize0">
			<a href="#" class="fb_share_project"><i class="fa fa-facebook"></i></a>
			<a href="#" class="twitter_share_project" data-title="<?php echo htmlspecialchars($open_bidding_project_value['project_title'], ENT_QUOTES);?>" data-description="<?php echo limitString(htmlspecialchars($open_bidding_project_value['project_description'], ENT_QUOTES), $this->config->item('twitter_share_project_description_character_limit'));?>"><i class="fa fa-twitter"></i></a>
			<a href="#" class="ln_share_project" data-title="<?php echo htmlspecialchars($open_bidding_project_value['project_title'], ENT_QUOTES);?>" data-description="<?php echo limitString(htmlspecialchars($open_bidding_project_value['project_description'], ENT_QUOTES), $this->config->item('facebook_and_linkedin_share_project_description_character_limit'));?>"><i class="fa fa-linkedin"></i></a>
			</div> -->
		<?php //} ?>
	</div>
	<!-- <div class="col-md-6 col-sm-6 col-12 default_applyNow_adjust">
		...
		<div class="row default_applyNow_btn">
			...
			?> 
				<button type="button" class="btn default_btn blue_btn apply_now_button newly_posted_apply_now_logged_in" data-attr="<?php echo $open_bidding_project_value['project_id']; ?>" data-page-type-attr = "listing" ><?php echo $apply_now_button_text; ?></button>
			<?php
				endif;
			?>
			</div>
...
<div class="initialViewNorecord">
    <?php echo $this->config->item('projects_notification_feed_page_search_no_results_returned_message'); ?>
</div>
<?php
    } 
?>